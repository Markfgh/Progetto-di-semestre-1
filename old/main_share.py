...
def _jet_rgba(norm: np.ndarray) -> np.ndarray:
    """
    Converte una matrice normalizzata [0,1] in RGBA float32 (0..1)
    colormap stile JET (approx).
    Output shape: (H, W, 4)
    """
    ...
    rgba = np.stack((r, g, b, np.ones_like(x)), axis=-1).astype(np.float32)
    return rgba
...
def dsp_worker(
    ...
    tex_w: int,
    tex_h: int,
    seq,            # mp.RawValue(c_uint32)  - seqlock counter
    active_idx,     # mp.RawValue(c_int)    - 0/1
    ui_vmin,
    ui_vmax,
    ui_rmax,
    ui_xmax,
    ...
    '''
    DSP worker:
      - legge i frame grezzi da shm_frames (come prima)
      - calcola heatmap (float32) come prima
      - converte la heatmap direttamente in una texture RGBA (float32) a dimensione fissa (tex_h, tex_w)
      - scrive la texture in shared memory DOUBLE BUFFER (2 buffer)
      - publish "latest-wins" tramite seqlock (seq + active_idx)

    Perché è realtime-correct:
      - nessuna Queue tra DSP e GUI => nessun backlog FIFO / niente pickle
      - latest-wins: la GUI mostra sempre l'ultimo frame pubblicato
      - no tearing: seqlock garantisce che la GUI copia solo frame completi
    '''
    # --- SETUP STATIC (NECESSARIO anche con spawn / Windows) ---
    ...
    # --- FIX SHAPE FINESTRE (fondamentale per broadcasting corretto) ---
    window_range = np.asarray(window_range).reshape(1, 1, 1, SAMPLES, 1)
    window_angle = np.asarray(window_angle).reshape(1, 1, 1, 1, VIRTUAL_ANT)

    dr_plot = C * FS / (2.0 * SLOPE * NFFT_RANGE)
    max_bin = int(np.floor(RANGE_MAX_DISPLAY / dr_plot))
    ...
    shm_tex = shared_memory.SharedMemory(name=shm_tex_name)
    try:
        tex_np = np.ndarray((2, tex_h, tex_w, 4), dtype=np.float32, buffer=shm_tex.buf)
        ...
        def _publish(buf_idx: int) -> None:
            seq.value = ctypes.c_uint32(seq.value + 1).value  # odd
            active_idx.value = int(buf_idx)
            seq.value = ctypes.c_uint32(seq.value + 1).value  # even
        ...
        while True:
            ...
            heatmap_ema = process_buffer_heatmap(
                complex_data,
                window_range,
                window_angle,
                heatmap_ema,
            ...
            src = heatmap_ema.astype(np.float32, copy=False)

            vmin = float(ui_vmin.value)
            vmax = float(ui_vmax.value)
            rmax = float(ui_rmax.value)
            xmax = float(ui_xmax.value)
            if vmax <= vmin:
                vmax = vmin + 1.0
            denom = vmax - vmin
            if denom < 1e-6:
            ...
            buf_idx = 1 - (int(active_idx.value) & 1)
            tex_np[buf_idx, :, :, :] = rgba
...
def process_buffer_heatmap(raw_buffer, w_range_1d, w_angle_1d, heatmap_ema, alpha, max_bin):
    data = raw_buffer.reshape(X_FRAMES, CHIRPS // TX, TX, SAMPLES, RX)
    data = data - np.mean(data, axis=3, keepdims=True)

    # range window (SAMPLES)
    w_range = np.asarray(w_range_1d).reshape(1, 1, 1, SAMPLES, 1)
    ...
    rng = rng[:, :, :, :max_bin, :]   # <-- questa riga resta identica ✅

    # ---- costruisci virtual array (TX*RX) e fai angle FFT lì ----
    # rng: (XF, C, TX, R, RX) -> (XF, C, R, TX*RX)
    rng_va = np.transpose(rng, (0, 1, 3, 2, 4)).reshape(X_FRAMES, CHIRPS // TX, max_bin, VIRTUAL_ANT)

    w_angle = np.asarray(w_angle_1d).reshape(1, 1, 1, VIRTUAL_ANT)
    ...
    pwr = np.abs(ang) ** 2
    heat = pwr.sum(axis=(0, 1))  # -> (R, angleBins)

    heat_db = 10.0 * np.log10(heat + 1e-12)
    heat_db -= float(np.max(heat_db))

    if heatmap_ema is None:
        return heat_db.astype(np.float32, copy=True)
    return (1.0 - alpha) * heatmap_ema + alpha * heat_db.astype(np.float32, copy=False)
...
def main():
    ...
    # --- GUI TEXTURE SHM (double buffer, latest-wins) ---
    dr_plot = C * FS / (2.0 * SLOPE * NFFT_RANGE)
    ...
    shm_tex = shared_memory.SharedMemory(create=True, size=2 * tex_w * tex_h * 4 * 4)
    shm_tex_name = shm_tex.name
    shm_seq = mp.RawValue('I', 0)
    shm_active = mp.RawValue('i', 0)
    ui_vmin = mp.Value('f', float(VMIN), lock=False)
    ui_vmax = mp.Value('f', float(VMAX), lock=False)
    ui_rmax = mp.Value('f', float(RANGE_MAX_DISPLAY), lock=False)
    ui_xmax = mp.Value('f', float(CROSSRANGE_MAX_DISPLAY), lock=False)
    ...
    p_dsp = Process(
        target=dsp_worker,
        args=(
            frame_q, free_slots, shm_frames,
            shm_tex_name, tex_w, tex_h,
            shm_seq, shm_active,
            ui_vmin, ui_vmax, ui_rmax, ui_xmax,
            gui_put_drops, frame_get_ok, gui_put_ok,
            stop_evt,
        ),
    )
    ...
    # dr_plot, tex_w, tex_h sono già definiti sopra (servono anche al DSP)
    ...
    # 4) Texture setup (buffer locale GUI: nessuna allocazione per frame)
    ...
    # 5) Callbacks
    def _shutdown():
        # SHM texture cleanup (owner = main process)
        try:
            shm_tex_gui.close()
        except Exception:
            pass
        try:
            shm_tex.close(); shm_tex.unlink()
        except Exception:
            pass
    ...
    # --- SHM attach (GUI side) ---
    shm_tex_gui = shared_memory.SharedMemory(name=shm_tex_name)
    shm_tex_np = np.ndarray((2, tex_h, tex_w, 4), dtype=np.float32, buffer=shm_tex_gui.buf)
    last_seq = 0
    ...
    try:
        while dpg.is_dearpygui_running():
            ...
            if ui_dirty and (now - ui_dirty_t) >= 0.08:  # ~12.5 Hz, cambia 0.05..0.15 come vuoi
                ...
                # publish UI params verso DSP (lock-free best-effort)
                ui_vmin.value = float(vis_vmin)
                ui_vmax.value = float(vis_vmax)
                ui_rmax.value = float(vis_rmax)
                ui_xmax.value = float(vis_xmax)
            ...
            # 2. GUI TEXTURE UPDATE (SHM double buffer, latest-wins, no alloc per frame)
            # Thread-safe:
            # - DearPyGui API chiamata SOLO nel render loop (main thread)
            # - DSP scrive in SHM, GUI copia in tex_buf e fa set_value qui
            s1 = shm_seq.value
            if (s1 & 1) == 0 and s1 != last_seq:
                for _ in range(3):
                    s1 = shm_seq.value
                    if (s1 & 1) != 0:
                        continue
                    idx = int(shm_active.value) & 1

                    # Copia SHM -> buffer locale (nessuna alloc)
                    np.copyto(tex_np, shm_tex_np[idx].reshape(-1))
