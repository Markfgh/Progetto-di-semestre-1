"""Coordinamento non-GUI delle scansioni SAR lineari.
...
class _ScanCancelled(ScanError):
...
class ScanEvent(str, Enum):
    """Evento visualizzabile della scansione, con dati in :class:`ScanStatus`.

    La GUI deve scegliere il testo localizzato in base a questo codice e usare
    i campi ``position_*``, ``completed`` e ``total`` per i valori dinamici.
    """
    ...
    MECHANICAL_SETTLING = "mechanical_settling"
...
class ScanStatus:
    ...
    event: ScanEvent = ScanEvent.READY
    total: int = 0
    completed: int = 0
...
class SarScanCoordinator:
    ...
    def start(self, plan: ScanPlan) -> None:
        """Valida il piano e avvia il worker senza bloccare il thread chiamante."""
        plan.validate()
        with self._lock:
    ...
    def cancel(self) -> None:
        with self._lock:
            ...
            if self._status.state is ScanState.COMPLETED:
                return
    ...
    def _run(self, plan: ScanPlan, cancel_event: threading.Event) -> None:
        """Esegue per ogni posizione l'ordine invariabile: cattura, moto, attesa.

        L'ultimo campione non richiede movimento: ciò evita un jog superfluo e
        rende il numero delle catture esattamente uguale a ``plan.positions``.
        """
        success = False
        try:
            self._begin_motion()
            ...
            for index, _target_steps in enumerate(targets):
                ...
                self._set_status(
                    ...
                    event=ScanEvent.CAPTURING_POSITION,
                ...
                if cancel_event.is_set():
                    raise _ScanCancelled()
                # ``wait_capture`` ritorna soltanto dopo flush/close del logger:
                # il contatore rappresenta quindi file persistiti, non comandi
                # CAPTURE semplicemente accettati dalla coda.
                ...
                self._set_status(
                    ...
                    event=ScanEvent.MOVING_TO_POSITION,
                ...
                self._set_status(
                    ...
                    event=ScanEvent.MECHANICAL_SETTLING,
                )
            ...
            self._set_status(state=ScanState.CANCELLED, event=ScanEvent.CANCELLED)
            ...
            if cancel_event.is_set():
                ...
                self._set_status(
                    ...
                    event=ScanEvent.INTERRUPTED,
                    error=str(exc),
                )
        finally:
            # Il controller meccanico va sempre rilasciato, anche se cattura,
            # movimento o settling falliscono; solo dopo si pubblica l'esito.
            try:
                ...
                if success:
                    self._set_status(
                        state=ScanState.FAILED,
                        event=ScanEvent.FINALIZATION_FAILED,
                        error=str(exc),
                    )
            else:
                if success:
